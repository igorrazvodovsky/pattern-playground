...
const taxonomyPath = resolve(root, 'docs/specs/storybook-taxonomy.md');
...
for (const dir of storyDirs) {
  if (!taxonomyContent.includes(`\`${dir}/\``) && !taxonomyContent.includes(`\`${dir}\``)) {
    fail(`src/stories/${dir}/ exists but is not mentioned in docs/specs/storybook-taxonomy.md`);
  }
}
...
while ((match = dirPattern.exec(taxonomyContent)) !== null) {
  ...
  try {
    if (!statSync(full).isDirectory()) {
      fail(`docs/specs/storybook-taxonomy.md references ${dirName}/ but it is not a directory in src/stories/`);
    }
  } catch {
    fail(`docs/specs/storybook-taxonomy.md references ${dirName}/ but it does not exist in src/stories/`);
  }
}
