...
`TODO:` Use `multiple` with [tags](../?path=/docs/primitives-tag--docs) for multi-select behavior.
...
- [Input](../?path=/docs/primitives-input--docs) — the text field half of the contract, without the popup
- [Select](../?path=/docs/primitives-select--docs) — the simpler counterpart for short, bounded sets already known to the actor

### Used by

- [Bounded choice](/patterns/bounded-choice) — this control's job
- [Command menu](/patterns/command-menu) — the combobox contract drives hierarchical navigation and command activation
- [Filtering](/patterns/filtering) — each filter chip's value-picker uses a combobox to narrow available values
- [Autocomplete](/patterns/autocomplete) — narrowing-as-you-type is the combobox contract applied to query suggestion
- <PatternRef slug="searching">Searching</PatternRef> — the entry phase of instant-results search (popover or dropdown variant) follows the combobox model
