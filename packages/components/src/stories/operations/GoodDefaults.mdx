...
<Meta title="Operations/Good defaults" tags={['activity-level:operation', 'atomic:pattern', 'role:pattern', 'mediation:individual']} />
...
- [Onboarding](../?path=/docs/activities-onboarding--docs) - Often sets the initial state and defaults for the application
- <PatternRef slug="actions/application/settings">Settings</PatternRef> - Where global defaults are managed and overridden
- <PatternRef slug="actions/application/suggestion">Suggestion</PatternRef> - A weaker form of default ("Did you mean X?") or a dynamic default
- Autofill — the mechanism for applying defaults to forms
- [Agency](/patterns/qualities/agency) — a default is the system making the small decision so the actor doesn't have to; the actor still has the final say but doesn't have to spend it on this field
- [Form](../?path=/docs/actions-application-form--docs) - The primary container where defaults are encountered
- <PatternRef slug="actions/application/data-entry#input-hints">Input hints</PatternRef> - The alternative when a default cannot be guessed
- [Localization](/patterns/foundations/localization) — locale detection as a source of sensible defaults
