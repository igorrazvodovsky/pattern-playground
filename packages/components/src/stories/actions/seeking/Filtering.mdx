import { Canvas, Story, Meta, Subtitle } from '@storybook/addon-docs/blocks';
import * as FilteringStories from './Filtering.stories.tsx';

<Meta of={ FilteringStories } tags={['activity-level:action', 'atomic:composition', 'role:pattern', 'lifecycle:seeking', 'mediation:individual']} />

{/* > 🙂 **Fun meter: 3/5**. Dependency. */}

# Filtering

The narrowing of a [collection](../?path=/docs/concepts-collection--docs) to [items](../?path=/docs/concepts-item--docs) that match the actor's criteria, balancing precision with the risk of hiding relevant results.

<Story of={ FilteringStories.Filtering } />

## Placement

- inline, popover, drawer...
- pairing with table header

## Navigation as filtering

...

## LLM-powered attribute search

When simple filter fails, the attribute search query is passed to LLM to find a match in the available attributes.

If that fails, users can delegate filtering to a <PatternRef slug="activities/bot">bot</PatternRef> via a [task](../?path=/docs/concepts-task--docs). Bot can have access to data beyond the attributes and values visible to the user.

### Flow 1

1. Enter a query that does not have a match in the available attributes
2. Create a task to handle it
3. Show "thinking..." placeholder
4. Replace it with a task result reference.

### Flow 2

1. Create a task outside filtering whose result can be used to filter the collection.
2. Follow steps 3-4 from Flow 1.

<Story of={ FilteringStories.LLMFilter } />

## Related patterns

### Precursors

- Typically placed in a [toolbar](../?path=/docs/components-toolbar--docs)
- Depends on [command menu](../?path=/docs/components-command-menu--docs) for the local search through attributes and values.
- [Combobox](../?path=/docs/operations-combobox--docs) — the value-picker inside each active filter chip uses a combobox to narrow available values


### Complementary

- [Grouping](../?path=/docs/actions-sensemaking-grouping--docs)
- [Sorting](../?path=/docs/actions-seeking-sorting--docs)
- [Selection](../?path=/docs/actions-coordination-selection--docs) — the imperative dual; filter narrows by predicate, selection narrows by enumeration
- Constrained natural language builder
- Rules
- Search

### Follow-ups

- [Data view](../?path=/docs/actions-seeking-data-view--docs)

## Resources & references

- Patternfly / [Filters](https://www.patternfly.org/patterns/filters/design-guidelines)
- Gitlab Pajamas / [Filtering](https://design.gitlab.com/patterns/filtering/)
- IBM Carbon / [Filtering](https://carbondesignsystem.com/patterns/filtering/)
- Salesforce Lightning / [Rules, filters, and logic](https://www.lightningdesignsystem.com/2e1ef8501/p/58ff5f-rules-filters-and-logic/)