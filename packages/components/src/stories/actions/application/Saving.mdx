...
<Meta
  title="Actions/Application/Saving"
...
This approach is particularly effective when deletion is part of a larger editing session. See <PatternRef slug="actions/application/deletion">Deletion</PatternRef> for detailed implementation examples.

## Related patterns

<div>
- <PatternRef slug="actions/application/deletion">Deletion</PatternRef> – Save/cancel workflows as confirmation for destructive operations
- <PatternRef slug="actions/application/action-consequences">Action consequences</PatternRef> – Evaluating when save/cancel is appropriate
- Draft vs. publish workflow
- <PatternRef slug="operations/undo">Undo</PatternRef> – Reversibility in saved states
- Multiplayer
- <PatternRef slug="activities/collaboration">Collaboration</PatternRef> – Saving patterns handle conflict resolution and multi-user editing scenarios in shared workspaces
</div>