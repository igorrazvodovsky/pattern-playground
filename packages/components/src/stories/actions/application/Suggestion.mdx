...
<Meta title="Actions/Application/Suggestion" tags={['activity-level:action', 'atomic:pattern', 'role:pattern', 'lifecycle:application', 'mediation:individual']} />
...
See [Conversation](../?path=/docs/activities-conversation--docs) dialogue forms for alternative interaction patterns that preserve diversity and elaboration depth.
...
- [Assistance](/patterns/foundations/assistance) — providing likely next actions and narrowing the option space
- [Delegation](/patterns/foundations/delegation) — agent might suggest opportunities it believes will help
- <PatternRef slug="actions/application/assisted-task-completion">Assisted task completion</PatternRef> — the completion spectrum, including <PatternRef slug="actions/application/ai-completion">AI completion</PatternRef> (inline generation) and [next-best action](../?path=/docs/actions-application-nextbest-action--docs) (workflow-level recommendations)
- [Adaptability](/patterns/qualities/adaptation) — suggestions might sharpen on the actor's accepted-or-rejected signal, fitting their patterns over time
- <PatternRef slug="activities/bot">Bot</PatternRef>
- [Messaging](../?path=/docs/actions-coordination-messaging--docs)
- [Transparent reasoning](../?path=/docs/activities-transparent-reasoning--docs) - suggestions can emerge from visible reasoning processes
- <PatternRef slug="activities/collaboration">Collaboration</PatternRef> - Suggestions support collaborative decision-making through AI-assisted discovery
- [Conversation](../?path=/docs/activities-conversation--docs) - dialogue forms that provide alternatives to suggestion-based interaction
- [Formality](/patterns/qualities/formality) — suggestions ease the step from informal intent to formal expression, formalising by recognition rather than recall
- When suggestions carry real stakes, <PatternRef slug="actions/application/cognitive-forcing-functions">cognitive forcing functions</PatternRef> offer interventions that preserve the user's judgement.
