...
<Meta title="Actions/Application/Wizard" tags={['activity-level:action', 'atomic:composition', 'role:pattern', 'lifecycle:application', 'mediation:individual']} />
...
Wizards sit at the intersection of <PatternRef slug="actions/seeking/progressive-disclosure">progressive disclosure</PatternRef> and [form](../?path=/docs/actions-application-form--docs) patterns, adding temporal sequencing and enforced dependencies. Where progressive disclosure reveals complexity gradually within a single context, wizards fragment the journey across multiple discrete steps, each gating access to the next.
...
Within the [Intent & Interaction framework](/patterns/foundations/interaction), wizards operate primarily in the **instructions for action** category, providing structured guidance that bridges actor uncertainty to task completion.

### From foggy finding to structured action

Wizards work best when actors face tasks that would otherwise trigger [uncovering or hunting behaviours](/patterns/foundations/interaction#foggy-finding)—they know what outcome they want but lack the structure to achieve it efficiently. By imposing a predetermined path, wizards convert potentially frustrating exploration into confident forward motion.

However, this same structure creates problems for actors in [navigating or browsing modes](/patterns/foundations/interaction#motivated-movement) who possess sufficient context to move purposefully. Forcing navigation-capable actors through sequential steps transforms their fluent behaviour into tedious compliance.
...
Unlike [conversation](../?path=/docs/activities-conversation--docs) patterns that adapt flexibly to actor intent, wizards impose monologic flow. The system dictates the sequence; actors follow.
...
### Precursors
- [Form](../?path=/docs/actions-application-form--docs) - the single-page structure that wizards fragment across steps
- <PatternRef slug="actions/seeking/progressive-disclosure">Progressive disclosure</PatternRef> - the conceptual foundation for revealing complexity gradually
- [Onboarding](../?path=/docs/activities-onboarding--docs) - often employs wizards for initial setup and configuration
- [Step by step](../?path=/docs/actions-navigation-step-by-step--docs) - Wizard is a specific implementation of the step by step navigation model, applying it to form-based tasks with validation and branching logic.

### Alternatives
- [Conversational form](../?path=/docs/actions-application-form--docs#conversational-forms): Embeds form inputs within [messaging](../?path=/docs/actions-coordination-messaging--docs) interface, allowing flexible dialogue whilst collecting structured data. Better for actors who benefit from clarifying questions.
- <PatternRef slug="activities/bot">Bot</PatternRef> - can guide through complex processes with conversational flexibility wizards lack

### Complementary
- [Status feedback](../?path=/docs/operations-status-feedback--docs) - communicates progress and validation state throughout the wizard
- <PatternRef slug="operations/undo">Undo</PatternRef> - mitigates the high commitment level by allowing reversal of completed steps
