...
- [Adaptability](/patterns/qualities/adaptation) — the system mixes rule-driven and inferred responses, so behaviour shifts with context rather than staying fixed
- [Agency](/patterns/qualities/agency) — the system decides and acts on its own behalf rather than only responding to the actor's explicit moves

### Follow-ups & Complements

- [Command menu](../?path=/docs/components-command-menu--docs): A natural home for embedded intelligence, turning a simple command line into an intent-driven interface.
...
- <PatternRef slug="actions/application/suggestion">Suggestion</PatternRef>: The output of embedded intelligence is often a suggestion that the user can accept, reject, or modify.
