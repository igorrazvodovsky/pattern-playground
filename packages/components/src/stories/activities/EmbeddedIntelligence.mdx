...
- [Adaptability](/patterns/adaptation) — the system mixes rule-driven and inferred responses, so behaviour shifts with context rather than staying fixed
- [Agency](/patterns/agency) — the system decides and acts on its own behalf rather than only responding to the actor's explicit moves
...
- [Filtering](/patterns/filtering): A prime example of embedded intelligence, where natural language queries are translated into structured filters.
- [Focus and context](/patterns/focus-and-context): Can use embedded intelligence to infer what contextual information is most relevant to a user's current focus.
- <PatternRef slug="suggestion">Suggestion</PatternRef>: The output of embedded intelligence is often a suggestion that the user can accept, reject, or modify.
