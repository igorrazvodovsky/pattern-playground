...
- <PatternRef slug="bot">Bot</PatternRef> — generates the stream that this surface presents in flight

### Follow-ups

- [Generated content](/patterns/generated-content) — once the stream completes, the move shifts from holding the surface stable to negotiating what arrived

### Complementary

- <PatternRef slug="transparent-reasoning">Transparent reasoning</PatternRef> — its "stream of thought" mode rides on the same rendering substrate
- [Conversation](/patterns/conversation) — live presentation is one of the rhythms a conversational turn can take
- [Abort](/patterns/abort) — the user-side stop depends on the surface having a quiet resting state to land in
