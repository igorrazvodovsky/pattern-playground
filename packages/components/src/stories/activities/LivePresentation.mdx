...
<Meta title="Activities/Live presentation" tags={['activity-level:activity', 'atomic:pattern', 'role:pattern', 'mediation:individual']} />
...
This is the *during-generation* phase that precedes [generated content](../?path=/docs/activities-generated-content--docs)'s post-generation negotiation. While the system is still emitting, the design problem isn't yet trust or refinement. It's whether the surface lets the actor read at all.
...
When the actor [aborts](../?path=/docs/operations-conversation-sequence-management-abort--docs) a stream, the surface should reach a quiet resting state. Clear the pending buffer, drop the cursor and status indicator, and surface a retry that keeps the original prompt intact.
...
- <PatternRef slug="activities/bot">Bot</PatternRef> — generates the stream that this surface presents in flight

### Follow-ups

- [Generated content](../?path=/docs/activities-generated-content--docs) — once the stream completes, the move shifts from holding the surface stable to negotiating what arrived

### Complementary

- <PatternRef slug="activities/transparent-reasoning">Transparent reasoning</PatternRef> — its "stream of thought" mode rides on the same rendering substrate
- [Conversation](../?path=/docs/activities-conversation--docs) — live presentation is one of the rhythms a conversational turn can take
- [Abort](../?path=/docs/operations-conversation-sequence-management-abort--docs) — the user-side stop depends on the surface having a quiet resting state to land in
