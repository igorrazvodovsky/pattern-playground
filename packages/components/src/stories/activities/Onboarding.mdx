...
<Meta title="Activities/Onboarding" tags={['activity-level:activity', 'atomic:pattern', 'role:pattern', 'mediation:individual']} />
...
- Delivery mechanisms: <PatternRef slug="activities/help">guided tours and overlays</PatternRef> introduce features without requiring pedagogic commands.
...
- *Enable <PatternRef slug="activities/activity-log">archaeology</PatternRef>:* Let users trace how things came to be—history, authorship, related changes
- *Identify local conventions:* What naming patterns, categorisations, or workflows are established here?

This is [domain learnability](/patterns/qualities/learnability) applied to accumulated context.
The system holds knowledge not just about the domain in general, but about *this organisation's* relationship to the domain.

## Related patterns

### Precursors
- [Learnability](/patterns/qualities/learnability) – the foundation supporting value demonstration.
- [Empty state](../?path=/docs/operations-empty-state--docs) — often the canvas for onboarding and as such an enabler for it

### Follow-ups
- <PatternRef slug="activities/mastery">Mastery</PatternRef> – building proficiency after initial learning.

### Complementary
- <PatternRef slug="activities/help">Help</PatternRef> – provides just-in-time assistance.
- [Localization](/patterns/foundations/localization) — first-run locale detection and selection
