...
Effective <PatternRef slug="collaboration">collaboration</PatternRef> often sequences dialogue forms—using questions during exploration phases and suggestions during refinement.
...
- [Conversation](/patterns/conversation-quality) — names the foundational stance this activity makes its primary mode: dialogue as the substrate of meaning, not one option among many

### Precursors

- [Localization](/patterns/localization) — dialogue structure adapted to cultural communication norms
...
- [Prompt](/patterns/prompt) translates user needs into structured AI behavior within a conversation
- <PatternRef slug="text-lense">Text lens</PatternRef> – filter lenses adapt conversational tone and style based on context and relationship
- *Conversation history* facilitates continuity, knowledge retention, and revisiting prior interactions
- <PatternRef slug="collaboration">Collaboration</PatternRef> – uses conversation as a core communication mechanism for collaborative work
- [Form](/patterns/form) can evolve into a conversation by adopting a dialogic flow
...
- Instruction-based dialogue typically begins with [open request](/patterns/open-request), where users make complex requests requiring clarification and multi-step execution
- All dialogue forms leverage [repair affordances](../?path=/docs/operations-conversation-sequence-management-bot-repair--docs) and [sequence management](/patterns/sequence-completion) patterns to handle breakdowns and closures
- Encounters are framed by conversation management primitives — [opening (bot)](../?path=/docs/operations-conversation-conversation-management-opening-bot--docs), [opening (user)](../?path=/docs/operations-conversation-conversation-management-opening-user--docs), [capability & scope](/patterns/capability-and-scope), [closing](../?path=/docs/operations-conversation-conversation-management-closing--docs), and [disengage without closing](/patterns/disengage-without-closing) — that structure the start, end, and interruption of encounters
