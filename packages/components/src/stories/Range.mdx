...
<Meta title="Components/Range" of={RangeStories} tags={['activity-level:operation', 'atomic:primitive', 'role:component', 'mediation:individual']} />
...
Children marked `data-slot="prefix"` / `data-slot="suffix"` hold units, endpoint labels, or icons. With `hide-value`, the default numeric readout is suppressed so the suffix can carry a formatted value instead.
...
### Related primitives
- [Input](../?path=/docs/components-input--docs) — typed numeric entry when the actor already knows the exact value.
- [Counter](../?path=/docs/utilities-counter--docs) — announces the numeric state a slider changes; useful as a passive readout alongside direct manipulation.

### Applied in
- <PatternRef slug="data-entry">Data entry</PatternRef> — the broader activity, where slider-vs-input is the key choice.
- [Form](/patterns/form) — the common container.
