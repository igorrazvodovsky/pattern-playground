...
relationships:
  ...
  composed-of:
    - to: agent-opening
      note: the bot speaks first — establishing footing and proposing the first topic
    - to: user-opening
      note: the conversation opens with the actor speaking first; the bot aligns to their topic
    - to: capability-and-scope
      note: an explicit statement of what the conversation can and cannot do
    - to: inquiry-agent
      note: the bot asks for information it lacks; the move that drives question-driven dialogue
    - to: open-request
      note: the actor makes a complex request needing clarification and multiple steps; where instruction-based dialogue begins
    - to: agent-repair
      note: the bot repairs its own misunderstanding — ambiguity, low confidence, missing detail
    ...
    - to: inquiry-user
      note: the actor asks and the bot answers directly — the minimal exchange, present in every dialogue form
    - to: extended-telling
      note: the bot holds the floor across several turns to instruct, narrate, or explain
    - to: abort
      note: the actor stops an unwanted or failing sequence mid-exchange — the conversational stop, not a process kill
    - to: user-repair
      note: the actor signals the bot's last utterance didn't land, repairing the exchange from the user's side
  enacts:
