...
fun: 🙂 5/5.
realised_by: [components-messaging--docs]
situation:
  initiating: >-
    an evolving exchange between actors where meaning has to be built up over
    multiple turns rather than stated in one
  resulting:
    - >-
      a dialogue structure that carries context forward, lets either party take
      a turn, and accommodates repair when alignment slips
relationships:
  complements:
    - to: collaboration
  ...
  composed-of:
    - to: agent-opening
      note: the agent speaks first — establishing footing and proposing the first topic
    - to: user-opening
      note: the conversation opens with the actor speaking first; the agent aligns to their topic
    - to: capability-and-scope
      note: an explicit statement of what the conversation can and cannot do
    - to: inquiry-agent
      note: the agent asks for information it lacks; the move that drives question-driven dialogue
    - to: open-request
      note: the actor makes a complex request needing clarification and multiple steps; where instruction-based dialogue begins
    - to: agent-repair
      note: the agent repairs its own misunderstanding — ambiguity, low confidence, missing detail
    - to: sequence-completion
      note: the actor marks a sequence as successfully done
    - to: disengage-without-closing
      note: either party steps away without a formal close, leaving the exchange ready to resume
    - to: closing
      note: ending the encounter cleanly, with room for last topics — closing the exchange, not a window
    - to: inquiry-user
      note: the actor asks and the agent answers directly — the minimal exchange, present in every dialogue form
    - to: extended-telling
      note: the agent holds the floor across several turns to instruct, narrate, or explain
    - to: abort
      note: the actor stops an unwanted or failing sequence mid-exchange — the conversational stop, not a process kill
    - to: user-repair
      note: the actor signals the agent's last utterance didn't land, repairing the exchange from the user's side
  enacts:
    - to: conversation-quality
      note: "the quality names the stance conversation makes its primary mode: dialogue as the substrate of meaning, not one option among many"
    - to: agency
...
Agent asks reflective, open-ended questions that prompt human thinking and elaboration rather than providing direct solutions.
...
2. Agent asks knowledge or analogy question to generate alternatives
3. Human selects direction and elaborates
4. Agent asks clarifying questions to increase specificity and usefulness
...
Agent provides explicit recommendations, examples, or alternative approaches for human consideration.
...
Agent executes direct commands or requests with minimal back-and-forth.
...
Agent challenges assumptions, questions reasoning, or presents opposing viewpoints to strengthen thinking.
...
- Question-driven dialogue chains [agent inquiry](/patterns/inquiry-agent) moves, where the agent asks questions to elicit human thinking.
...
- Encounters are framed by conversation management primitives that structure their start, end, and interruption: [opening (agent)](/patterns/agent-opening), [opening (user)](/patterns/user-opening), [capability & scope](/patterns/capability-and-scope), [closing](/patterns/closing), and [disengage without closing](/patterns/disengage-without-closing).
...
- <ComponentRef id="components-messaging--docs">Messaging</ComponentRef> – where conversation happens
