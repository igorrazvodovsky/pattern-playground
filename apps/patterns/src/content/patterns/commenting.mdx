...
situation:
  initiating: >-
    the content is under review and the discussion about it belongs to particular places
    inside it rather than to the whole; a separate channel loses the anchor, and editing the
    content to raise the point destroys the thing being discussed
  resulting:
    - >-
      discussion is anchored where it applies and stays out of the content itself, so the
      primary reading survives an active review
    - clause: >-
        comments acquire a lifecycle of their own — open, answered, resolved — and something
        has to say when a thread is finished, or the layer only ever grows
      sets-up: [status-feedback]
    - >-
      the thread beside the content is a second place where decisions get made, and whatever
      was settled there is invisible to anyone reading only the content
relationships:
  complements:
    - to: collaboration
...
<Demo name="commenting" surface="white" label="Commenting via bubble menu" />
