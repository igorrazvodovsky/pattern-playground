...
relationships:
  ...
  follows:
    - to: hub-and-spoke
      note: 'Workspaces often implement hub and spoke navigation, where a workspace switcher or dashboard (hub) provides access to individual workspaces (spokes). See [Information Architecture foundation](/patterns/information-architecture) for the broader structural pattern.'
    - to: navigation-overview
      note: navigation enables movement through the application structure; workspaces extend it with parallel access to multiple destinations
  precedes:
    - to: activity-log
      note: shows history within a workspace
    - to: notification
      note: alerts actors to changes in non-visible workspaces
  related:
