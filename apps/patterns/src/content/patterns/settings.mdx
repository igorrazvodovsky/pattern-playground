...
situation:
  initiating: >-
    the defaults serve most actors and this one isn't among them on some axis they care
    about enough to go looking; the difference is a preference rather than a pattern, so
    no amount of watching their behaviour would have inferred it
  resulting:
    - >-
      the actor's overrides are recorded and the system holds to them, so an adaptation
      made once outlasts the session that made it
    - >-
      the surface accumulates: every option added is one more thing to look past, and the
      ones nobody ever changes still cost the actors hunting for the ones they do
    - >-
      a preference set once and honoured indefinitely stops being visible to the actor who
      set it, so behaviour that follows from it reads later as a fault rather than as their
      own earlier decision
    - clause: >-
        in shared configuration, one actor's change alters what others see without their
        doing anything — something has to carry the change across to them
      sets-up: [notification]
relationships:
  hosts:
    - to: localization
      note: locale controls live among the preferences
  precedes:
    - to: undo
      note: a recorded override is state undo can walk back, which is what makes exploratory configuration safe
    - to: status-feedback
      note: a changed preference is the material feedback confirms back to the actor
  enacts:
    - to: agency
      note: the actor overrides the defaults once and the system holds to it
    - to: privacy
      note: the actor sets who sees what once, and the boundary persists
    - to: adaptation
      note: the actor adapts the system to their context by hand, rather than leaving it to inference
  complements:
    - to: progressive-disclosure
      note: manages complexity in extensive settings through hierarchical revelation
    - to: form
      note: settings are essentially configuration forms requiring similar patterns (validation, feedback, submission)
    - to: wizard
      note: may guide actors through initial settings configuration during setup
    - to: onboarding
      note: settings can be introduced gradually during first-time experience
    - to: ai-tuning
      note: specialized settings for configuring AI behaviour
  tangential:
    - to: workspace
      note: settings affect workspace behaviour and may be scoped to workspace contexts
    - to: collaboration
      note: settings include collaboration preferences and affect shared work
---
...
Settings implement [agency](/patterns/agency) through configuration control, respect [privacy](/patterns/privacy) through visibility and access preferences, and enable [adaptability](/patterns/adaptation) by allowing actors to shape system behaviour to their contexts.

## Behavioural position

Within the [intent & interaction](/patterns/interaction) framework, settings operate across multiple behavioural categories depending on actor familiarity and intent.

### Navigating for experienced actors

When actors know exactly which setting they need, settings support efficient [navigating behaviour](/patterns/interaction#navigating). Clear organization, search functionality, and predictable structure enable direct access. Recent settings and favourites support swift re-finding.

### Exploring for discovery

New actors or those investigating available options engage in [exploring behaviour](/patterns/interaction#exploring). Progressive disclosure, logical grouping, and helpful descriptions support this discovery mode without overwhelming. Settings can serve as educational space—revealing capabilities through configuration options.

### Sense-making and integration

Settings facilitate [sense-making](/patterns/interaction#sense-making--integration) by organizing system capabilities into comprehensible structure. Actors build mental models of what's configurable and how different options relate. This understanding extends beyond immediate configuration tasks—settings documentation becomes reference material for understanding system behaviour.
...
Individual actor preferences visible and modifiable only by that actor. These implement [privacy](/patterns/privacy) through individual control and don't require coordination.
...
## Settings as [onboarding](/patterns/onboarding)
