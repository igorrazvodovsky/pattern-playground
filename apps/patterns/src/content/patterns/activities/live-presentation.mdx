...
When the actor [aborts](/patterns/operations/conversation/sequence-management/abort) a stream, the surface should reach a quiet resting state. Clear the pending buffer, drop the cursor and status indicator, and surface a retry that keeps the original prompt intact.
...
- [Abort](/patterns/operations/conversation/sequence-management/abort) — the user-side stop depends on the surface having a quiet resting state to land in
