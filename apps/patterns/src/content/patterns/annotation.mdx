...
situation:
  initiating: >-
    the content is fixed and not the actor's to change — someone else's document, a
    published record, a source that has to stay as it is — but their reading of it needs
    somewhere to live; without a layer over the top, interpretation either alters the
    original or stays in the actor's head
  resulting:
    - >-
      the actor's interpretation sits over the content without touching it, and the marks
      accumulate as wear — traces of where attention and effort actually went, readable
      long after
    - clause: >-
        once the marks are visible to others the layer stops being a private reading aid:
        other people read the actor's attention, and a mark becomes something that can be
        answered rather than only added to
      sets-up: [commenting]
    - >-
      the layer competes with what it annotates; past some density the marks are what the
      reader sees, and pruning them is nobody's job by default
relationships:
  enables:
    - to: collaboration
      note: a shared mark is one of the small moves collaborative work is built from
  enacts:
    - to: temporality
