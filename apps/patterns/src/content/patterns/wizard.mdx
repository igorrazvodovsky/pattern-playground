...
fun: 😐 2/5. Necessary infrastructure.
situation:
  initiating: >-
    the task is one atomic goal made of genuinely dependent steps — what's entered early
    determines what's available later — and the actor does it rarely enough not to hold its
    structure; a single surface would have to show options that don't apply yet
  resulting:
    - >-
      one commitment sits in front of the actor at a time, each step gated on the last, and
      because the branching prunes as it goes the path they walk is shorter than the whole
    - clause: >-
        the corridor removes peripheral vision: the actor can't compare across steps or jump
        to the one they know they need, so a change of mind partway has to be able to walk
        backwards without discarding the sequence
      sets-up: [undo]
    - >-
      the steps are the system's model of the task, and an expert who already holds that model
      experiences the guidance as compliance
relationships:
  alternative:
    - to: next-best-action
      note: the wizard prescribes the sequence; next-best action is the adaptive, context-aware alternative
    - to: form
      note: the single-page structure that wizards fragment across steps
    - to: agent
      note: can guide through complex processes with conversational flexibility wizards lack
  complements:
    - to: status-feedback
      note: communicates progress and validation state throughout the wizard
    - to: undo
      note: mitigates the high commitment level by allowing reversal of completed steps
  enacts:
    - to: temporality
      note: paces a task into an ordered sequence of steps, one commitment at a time
---
...
Wizards sit at the intersection of [progressive disclosure](/patterns/progressive-disclosure) and [form](/patterns/form) patterns, adding temporal sequencing and enforced dependencies. Where progressive disclosure reveals complexity gradually within a single context, wizards fragment the journey across multiple discrete steps, each gating access to the next.
...
Within the [intent & interaction](/patterns/interaction) framework, wizards operate primarily in the **instructions for action** category, providing structured guidance that bridges actor uncertainty to task completion.

### From foggy finding to structured action

Wizards work best when actors face tasks that would otherwise trigger [uncovering or hunting behaviours](/patterns/interaction#foggy-finding)—they know what outcome they want but lack the structure to achieve it efficiently. By imposing a predetermined path, wizards convert potentially frustrating exploration into confident forward motion.

However, this same structure creates problems for actors in [navigating or browsing modes](/patterns/interaction#motivated-movement) who possess sufficient context to move purposefully. Forcing navigation-capable actors through sequential steps transforms their fluent behaviour into tedious compliance.
...
Unlike [conversation](/patterns/conversation) patterns that adapt flexibly to actor intent, wizards impose monologic flow. The system dictates the sequence; actors follow.
...
- [Conversational form](/patterns/form#forms-and-conversation): Embeds form inputs within <ComponentRef id="components-messaging--docs">messaging</ComponentRef> interface, allowing flexible dialogue whilst collecting structured data. Better for actors who benefit from clarifying questions.
