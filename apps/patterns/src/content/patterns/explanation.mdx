...
situation:
  initiating: >-
    the system did something the actor didn't expect or can't act on — a rule fired, an
    option isn't there, a recommendation arrived — and the reason for it lives inside the
    system rather than on the surface; left there, the actor either guesses at it or defers
    to the system wholesale
  resulting:
    - >-
      the reasoning is legible, so the actor can judge when to trust the system and when to
      override it — which is a different capability from being able to operate it
    - clause: >-
        a complete and plausible explanation can make deferring *easier* rather than
        harder: the actor now holds a justification they didn't have to think through, which
        is the state a forcing function exists to interrupt
      sets-up: [cognitive-forcing-functions]
    - >-
      an explanation pitched at the actor's level is a guess about that level — too much
      depth reads as noise, too little as evasion, and the actor can't ask for the
      difference unless the depth is theirs to move
relationships:
  related:
    - assistance
    - state-disabled
  ...
  complements:
    - to: transparent-reasoning
      note: shows step-by-step process while explanation encourages understanding through interaction
    - to: collaboration
      note: uses explanations to facilitate understanding through transparency about other actors' actions
  enacts:
    - to: learnability
      note: explanations teach the domain the tool encodes, not just how to operate it
    - to: agency
      note: makes the system's reasoning legible so the actor can judge when to defer and when to override
    - to: adaptation
      note: explanations adjust their depth and form to the actor's level and context
---
...
In complex systems, the interface embeds domain knowledge—process logic, regulatory requirements, industry practices.
...
Domain explanations transform system constraints from arbitrary-feeling to meaningful. They support [domain learnability](/patterns/learnability)—helping users understand the domain through the tool.
...
<Demo name="dynamic-explanation" surface="white" label="Dynamic explanation via bubble menu" />
...
Users can access the explanation <ComponentRef id="components-popover--docs">popover</ComponentRef> by clicking the explanation indicator. This popover offers contextual information beyond a single value and lists the factors that contributed to the result.
...
TODO: Example: a <ComponentRef id="components-drawer--docs">drawer</ComponentRef> with instructions or help text.

## Resources

- Andy Matuschak (2025) [How might we learn?](https://andymatuschak.org/hmwl/) – contextualised study and dynamic explanations grounded in user's actual work
