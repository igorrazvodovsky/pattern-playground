...
situation:
  initiating: >-
    the content is densely cross-referenced and every link is a fork in the actor's
    attention; finding out whether a link was worth following costs the reading
    position, so the actor either clicks indiscriminately or stops clicking
  resulting:
    - >-
      the actor can sample a destination without leaving their place, so triage
      happens at the link rather than after the navigation — follow it, skip it, or
      note it for later
    - >-
      the page now owes the actor a ladder of commitment rather than one click:
      peek, open alongside, open fully — and each rung has to be reachable by
      keyboard as well as by hover, since hover is the one channel touch doesn't have
relationships:
  complements:
...
- <ComponentRef id="components-popover--docs">Popover</ComponentRef> — the surface primitive; link preview is one specific application of its hover/focus mode
- <ComponentRef id="components-reference--docs">Reference</ComponentRef> — references reify connections as first-class entities; their preview escalation (hover → detail → full view) is one application of the link preview move within that broader pattern
