...
situation:
  initiating: >-
    a moment where the actor must designate members of an exposed set of options
  resulting:
    - >-
      a designation — narrowed by scanning or typing — whose fate (committed
      value, staked target, spliced text) is set by the surrounding flow
relationships:
  enables:
    - to: deletion
      note: the most common operation routed through selection; confirmation strategy should scale with the selection count
    - to: tag
      note: a selection is the common payload of bulk tagging
    - to: grouping
      note: a selection is the common payload of bulk grouping
  related:
    - to: searching
  ...
  # reverse-reading note on data-view's `enables` edge, not a new edge
  composed-of:
    - to: data-view
      note: the view in which item selection happens — it supplies the visible set the pick is drawn from
---

import { InlineCheckboxesDemo, ModeBasedRevealDemo, SelectAllAffordanceDemo } from '@pkg/demos/selection';
...
- Selection lives with the viewport — anything not currently visible is removed from the selection. Acceptable when selections are short-lived and small.
...
<Demo surface="gray" label="Select-all with escalation to the matching set">
  <SelectAllAffordanceDemo client:only="react" />
</Demo>
...
<Demo surface="gray" label="Mode-based reveal">
  <ModeBasedRevealDemo client:only="react" />
</Demo>
...
- <ComponentRef id="components-checkbox--docs">Checkbox</ComponentRef> — the most explicit selection affordance
- <ComponentRef id="components-block-based-editor--docs">Block-based editor</ComponentRef> — the surface in which content selection happens
