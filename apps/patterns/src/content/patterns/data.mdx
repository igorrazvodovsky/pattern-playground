...
role: foundation
fun: 🤯 5/5. Exploring how AI-powered software operates on layered data architectures where deterministic foundations are augmented with probabilistic layers.
relationships:
  enacts:
    - to: agency
      note: the actor tunes the layering by hand rather than accepting the defaults
    - to: adaptation
      note: hard facts stay fixed while the soft, inferred layers flex to context
    - to: conversation-quality
      note: the inferred layers are offers, not facts — the record settles as the actor accepts or rejects them
    - to: privacy
      note: the data layer governs what information can be perceived, and by whom
  precedes:
    - to: dynamic-hyperlinks
      note: hard and soft data layering
---
