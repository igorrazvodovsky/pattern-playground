...
- [Onboarding](/patterns/onboarding) - Often sets the initial state and defaults for the application
- [Settings](/patterns/settings) - Where global defaults are managed and overridden
- [Suggestion](/patterns/suggestion) - A weaker form of default ("Did you mean X?") or a dynamic default
- Autofill — the mechanism for applying defaults to forms
- [Agency](/patterns/agency) — a default is the system making the small decision so the actor doesn't have to; the actor still has the final say but doesn't have to spend it on this field
- [Form](/patterns/form) - The primary container where defaults are encountered
- [Input hints](/patterns/data-entry#input-hints) - The alternative when a default cannot be guessed
- [Localization](/patterns/localization) — locale detection as a source of sensible defaults
