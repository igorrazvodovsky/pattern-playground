...
fun: 🙂 2/5. Included as a prerequisite for more interesting things.
situation:
  initiating: >-
    an actor's first encounter with a system (or a changed part of one) before
    they have a working mental model of what's possible and what will happen
  resulting:
    - >-
      a bounded period in which the actor can build that model with low stakes,
      often by doing rather than reading
relationships:
  precedes:
    - to: agent
      note: the first steps that help people get to know the agent — what it can and can't do
    - to: mastery
      note: building proficiency after initial learning
  composed-of:
    - to: wizard
      note: often employs wizards for initial setup and configuration
    - to: template
      note: starter templates give the newcomer competent work to occupy
  enacts:
    - to: learnability
      note: the actor's first climb up the learning curve, deliberately shaped
    - to: adaptation
      note: scaffolding that adapts to the newcomer, heavier at first and fading as competence grows
    - to: temporality
      note: the early, high-slope phase of the actor's arc with the tool
  complements:
    - to: help
      note: provides just-in-time assistance.
    - to: localization
...
- Make agency legible: if automations/agents will act, show what they can do, when they'll do it, and how to override.
- Delivery mechanisms: [Guided tours and overlays](/patterns/help) introduce features without requiring pedagogic commands.
...
- *Enable [archaeology](/patterns/activity-log):* Let users trace how things came to be—history, authorship, related changes
- *Identify local conventions:* What naming patterns, categorisations, or workflows are established here?

This is [domain learnability](/patterns/learnability) applied to accumulated context.
