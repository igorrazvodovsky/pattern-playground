...
situation:
  initiating: >-
    the actor is changing something that has to outlast the session, and when it becomes
    real is a decision the interface makes for them — commit continuously and the moment
    of intent disappears; wait for them and the work is what's at risk
  resulting:
    - >-
      there is a defined moment when changes become real and the actor can tell which side
      of it they are on, which is what makes a review step before committing possible at all
    - >-
      the commit boundary is where conflicts surface: two actors working on the same thing
      find out only when one of them makes changes real
    - >-
      autosaved work carries no record of what the actor meant to keep, so going back
      becomes the only substitute for having not committed
relationships:
  related:
    - to: deletion
...
This approach is particularly effective when deletion is part of a larger editing session. See [Deletion](/patterns/deletion) for detailed implementation examples.
