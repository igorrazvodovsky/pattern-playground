...
relationships:
  precedes:
    ...
    - to: conversation
      note: messaging is the primary interface for conducting bot conversations
    - prompt
    - ai-tuning
  ...
  complements:
    - to: collaboration
      note: Bots participate as collaborative agents with adaptable mediation capabilities
  enacts:
    - to: agency
      note: how much freedom the bot has to act on its own.
    - to: formality
      note: bots accept unstructured input by design; structure can be inferred or gradually formalised from interaction.
    - to: learnability
      note: the bot lets the actor ask the system rather than decipher it; learning happens through dialogue at the actor's pace
    - to: adaptation
      note: the bot modulates its behaviour and output to context and inferred intent rather than following fixed rules
  related:
    - ai-completion
    - next-best-action
---
