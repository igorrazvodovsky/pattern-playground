...
role: pattern
evidence:
  - { kind: literature, ref: collaboration-through-agency }
...
situation:
  initiating: >-
    several actors are working toward one outcome and each can only see their own part; with no
    shared account of what is happening they duplicate work, overwrite each other, and lose
    track of who owes what to whom
  resulting:
    - >-
      work that was independent becomes joint: participants can see what others did, follow the
      reasoning behind it, and converge without needing to meet
    - >-
      agency is distributed and renegotiated turn by turn rather than held by anyone, so who
      decides is a live question at every step rather than a settled one
    - >-
      coordination now runs in a rhythm — synchronous or asynchronous — and that rhythm decides
      what kinds of disagreement can be resolved at all
relationships:
  enacts:
    - to: agency
      note: agency is distributed, traded, and renegotiated turn by turn; human-centric locus tends to deepen elaboration, system-centric locus tends to surface acceptance
    - to: temporality
      note: joint work runs in synchronous or asynchronous time; the rhythm of coordination is itself a design choice
  complements:
    - to: activity-log
      note: the shared record that lets participants reconstruct what happened
    - to: notification
      note: signals what changed, by whom, and whether actor should look
    - to: explanation
      note: how participants understand each other's reasoning, not just each other's actions
    - to: suggestion
      note: "co-creation in tentative form: options participants can converge on or reject without commitment"
    - to: transparent-reasoning
      note: makes the basis of decisions auditable
    - to: command-menu
      note: keeps collaboration commands accessible without crowding the surface
    - to: agent
      note: frames the AI as a collaborator with its own modes
    - to: conversation
      note: how collaborators reach understanding and commitment
  instantiates:
    - to: collaboration-foundation
      note: the foundation the collaboration activity realises
---
...
See [Collaboration](/patterns/collaboration-foundation) foundation for conceptual grounding.
...
Human-AI collaboration unfolds through the interaction stages with shifting [agency patterns](/patterns/agency#spectrum), [distribution](/patterns/agency#distribution), and [control mechanisms](/patterns/agency#control-patterns).
...
- Risk of [algorithmic loafing](/patterns/agent#algorithmic-loafing)—reduced human effort when AI takes creative lead
...
- [Agency distribution](/patterns/agency#distribution): Human-centric locus enables deeper elaboration; system-centric locus often correlates with surface acceptance
...
See [Conversation](/patterns/conversation#dialogue-forms) for how dialogue forms shape elaboration in conversational interfaces, and [Agent](/patterns/agent) for interaction patterns that maintain engagement.
