---
title: "Intent & Interaction"
added: 2025-05-14
updated:
role: foundation
fun: "🧪 4/5. Exploring how to design for LLMs brought actor intent into focus, calling for a some kind of framework. Not super happy about the current state. I.e. it makes sense but not very useful in practice (yet)."
relationships:
  related:
    - to: assistance
      note: >-
        the kinds of support a system can offer at each stage of the actor's cognitive cycle (perceiving, knowing,
        planning, performing, reflecting).
    - to: delegation
      note: the lifecycle and touchpoints of agentive systems.
    - information-architecture
    - to: fully-connected
      note: maximum efficiency for precise movement
    - to: multilevel-tree
      note: efficient when mental model matches structure
    - to: pyramid
      note: hub enables direct targeting
    - to: step-by-step
      note: guided completion without distraction
    - to: flat-navigation
      note: sustained focus on work
    - to: pan-and-zoom
      note: spatial exploration feels natural
    - to: hub-and-spoke
      note: avoid opaque hub-and-spoke implementations
    - to: searching
      note: search read as a navigation behaviour — how the seeking move fits the actor's broader movement through the system
  enacts:
    - to: formality
      note: 'parallels the arc: as engagement matures, so does the structure around it, loose to precise'
    - to: adaptation
      note: the engagement arc reshapes itself as intent sharpens; what the system offers shifts stage by stage
    - to: agency
      note: each loop has a driver — actor-led with the system assisting, or system-led with the actor returning at touchpoints
    - to: temporality
      note: each see–think–do loop sets a beat; the loops compound into the longer arc
---
...
- When the human stays in the loop and the system helps at each phase, the relevant frame is [assistance](/patterns/assistance) — the kinds of support a system can offer at each stage of the actor's cognitive cycle (perceiving, knowing, planning, performing, reflecting).
- When the system runs the loop in the background and the human returns at discrete moments, the relevant frame is [delegation](/patterns/delegation) — the lifecycle and touchpoints of agentive systems.
...
Resolution state: intent *emerges* — neither party is yet committed. See [formality](/patterns/formality) on early-stage looseness.
...
Resolution state: intent *reflects* — outcomes are evaluated against the original need. Feedback loops into future intent formation. [Adaptability](/patterns/adaptation) matters most here: systems that learn from this stage shape better relationships next time.
...
Each stage of engagement involves different kinds of [activity](/patterns/agency#agency-across-activity-types-and-intent-resolution) with different agency [distributions](/patterns/agency#distribution). The two axes cross freely.

## Navigation

The intent relationship expressed as movement. Two dimensions shape navigation behaviours: *intent resolution* (how clear is the goal?) and *structural mapping* (how well does the [information architecture](/patterns/information-architecture) match the actor's mental model?).
...
- [Fully connected](/patterns/fully-connected) — maximum efficiency for precise movement
- [Multilevel tree](/patterns/multilevel-tree) — efficient when mental model matches structure
- [Pyramid](/patterns/pyramid) — hub enables direct targeting
...
- [Pyramid](/patterns/pyramid) — overview enables pattern recognition
- [Multilevel tree](/patterns/multilevel-tree) — structured exploration within sections
...
- [Step by step](/patterns/step-by-step) — guided completion without distraction
- [Flat navigation](/patterns/flat-navigation) — sustained focus on work
...
- [Pan and zoom](/patterns/pan-and-zoom) — spatial exploration feels natural
- [Fully connected](/patterns/fully-connected) — freedom to follow curiosity
...
- [Multilevel tree](/patterns/multilevel-tree) — hierarchy provides memory cues
...
- [Fully connected](/patterns/fully-connected) — transparency reduces opacity
- Avoid opaque [hub and spoke](/patterns/hub-and-spoke) implementations
...
- [Fully connected](/patterns/fully-connected) — multiple navigation paths available
- TODO: Search

## Cross-cutting qualities

Several qualities cut across everything this page describes. They apply at every level — within a single turn around the [interaction cycle](#interaction-cycle), across any stage of the [engagement arc](#how-engagement-unfolds-over-time), and across any [delegation](/patterns/delegation) touchpoint. They are orthogonal to both the loop and the arc: any loop phase and any arc stage can involve any distribution of any of them.

- [Agency](/patterns/agency) — who is driving at any given moment, and what the system contributes at each stage
- [Temporality](/patterns/temporality) — the rhythm and pacing of turn-taking
- [Formality](/patterns/formality) — parallels the arc: as engagement matures, so does the structure around it, loose to precise

Activity types ([agency across activity types and intent resolution](/patterns/agency#agency-across-activity-types-and-intent-resolution)) describe what kind of work is happening. The arc tracks *how committed the relationship is*; activity types track *what kind of work* is happening within it. The two axes cross freely — an actor whose relationship with the system is still forming may be expressing a query, perceiving results, and thinking about criteria within a single stage.
