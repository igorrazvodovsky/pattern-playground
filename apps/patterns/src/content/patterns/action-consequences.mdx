...
relationships:
  related:
    - to: deletion
      note: "its decision tree is this framework projected onto one action — the tree's first question is the time-to-recover dimension"
    - to: saving
      note: "staging and drafts are severity reducers: they buy a review step before consequences commit"
    - to: undo
      note: "the strongest severity reducer — recovery after the act removes the need for confirmation before it"
    - to: inline-confirmation
      note: "the mid-ladder rung: enough friction to interrupt autopilot without breaking context"
---

import { ConsequenceLadderDemo } from '@pkg/demos/action-consequences';
...
### [Inline confirmation](/patterns/inline-confirmation)
...
<Demo label="Four actions, guarded in proportion to their consequences.">
  <ConsequenceLadderDemo client:only="react" />
</Demo>
