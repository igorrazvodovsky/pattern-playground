...
- Hiding options is an alternative to [disabled state](/patterns/state-disabled)
- [Morphing controls](/patterns/morphing-controls) — keeps the control visible but transforms it
- [Progressive disclosure](/patterns/progressive-disclosure) — a softer form: deferring rather than removing
- [Toolbar](/patterns/toolbar) — primary context where actions are hidden based on selection or state
- [Command menu](/patterns/command-menu) — context menus apply the same principle, surfacing only relevant commands