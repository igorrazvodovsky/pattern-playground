...
- Shareability strengthens [user agency](/patterns/agency) by making system state explicit and controllable. Users can precisely communicate their view, bookmark moments for later, and verify that "what they see is what they share." URLs become user-accessible state inspectors rather than opaque technical addresses.
- URLs function as [conversational interfaces](/patterns/conversation-quality)—they're contracts between application and user that communicate intent clearly. A well-designed URL answers "what am I looking at?" just by being read. This transparency builds trust and supports cooperative interaction.

- [Collaborative work](/patterns/collaboration-foundation) benefits when participants can share precise application states through URLs.


- [Adaptive systems](/patterns/adaptation) benefit from URL state as a mechanism for user-driven personalisation. View preferences, feature toggles, and configuration encoded in URLs let users experiment with adaptations and share successful configurations with others.

- Shareable URLs improve [accessibility](/patterns/a11y) by enabling users to bookmark and return to specific configurations (high contrast, increased density, focus modes). State persistence through URLs ensures assistive preferences aren't lost between sessions.
- [Localization](/patterns/localization) — locale-aware URLs and shared content
