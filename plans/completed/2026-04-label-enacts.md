---
title: "Label task — enacts edges"
status: "completed"
kind: "work-queue"
created: "2026-04"
last_reviewed: "2026-05-01"
area: "graph"
promoted_to: "docs/language/relationship-vocabulary.md"
superseded_by: ""
---
# Label task — enacts edges

Part of [label-queue.md](./2026-04-label-queue.md). 89 edges. Largest task; do in batches.
...
- *Batch 5 (foundations)* — done 2026-04-26. Eleven missing labels authored across seven foundation pages (`assistance → agency`, `collaboration → adaptability`, `delegation → agency`, `information-architecture → agency/shareability`, `intent-interaction → formality/adaptability/agency/temporality`, `modality → agency/temporality`, `material-layout → density`). Restructuring touched most of the pages: paragraph-form Related-patterns sections (Assistance, Delegation) converted to bulleted form; Information Architecture's nested prose-bullet list flattened into single-link bullets; Intent & Interaction's `## Cross-cutting qualities` bullets converted from `[Quality] does X` shape to `[Quality] — does X` shape and a new adaptability bullet added; Modality and Material/Layout gained `### Enacted qualities` subsections (Layout had no Related-patterns section at all). No new override candidates; the 13 already-labelled foundation edges read substantively.
- *Batch 6 (operations)* — done 2026-04-26. All six edges already had labels; two thin restatements rewritten in place (`operations-sections → qualities-adaptability`, `operations-good-defaults → qualities-agency`).

All six batches complete. Enacts label queue is closed: 88/88 edges carry substantive labels.
