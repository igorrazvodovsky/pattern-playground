...
- *`concepts/` folder*: the extractor skips the entire `Concepts` category (line 838). These 18 files are invisible to the graph. Removed from plan scope — concepts don't belong in either workspace in their current form and will be handled separately outside this migration.
...
## 5. Astro Lit-renderer prototype — complete ✅

Built at `apps/patterns/` (2026-05-15). Verified against production build.

*Setup:* Astro 6.3.3 with `@astrojs/lit` integration. `register-all.ts` imported via a `<script type="module">` in the layout, using a Vite alias (`@components → ../../src/components`). `ClientRouter` wired for view transitions (imported from `astro/components/ClientRouter.astro`).

*Results (production build — `astro preview`):*

- `customElements.get('pp-spinner')` → `true` on both home and about pages.
- `<pp-spinner>` count = 1 on both pages; element renders with full shadow DOM (the spinner SVG is visible).
- Zero console errors.
- Client-side navigation via `ClientRouter` works; element is defined and renders after transition.

*Dev-mode note:* Vite's first-cold-start dep optimization produces CSS MIME errors (504 → CSS-as-module race). These are transient — they resolve after the dep cache warms. The production build has no such issue; CSS `?inline` imports are bundled correctly as JS modules. No action needed before Phase B.

*Prototype files:*
- `apps/patterns/package.json`
- `apps/patterns/astro.config.mjs`
- `apps/patterns/tsconfig.json`
- `apps/patterns/src/layouts/Base.astro`
- `apps/patterns/src/pages/index.astro`
- `apps/patterns/src/pages/about.astro`

---

## Phase A verdict — closed ✅

All four questions are resolved. Phase B (workspace restructure) may proceed.

Action items carried into Phase D:

1. *Role corrections:* re-tag `Toast.mdx` from `role:component` to `role:pattern`; create `InlineConfirmation.mdx` with `role:pattern`; add explicit role frontmatter to all `qualities/` and `foundations/` files during migration.
2. *Combobox split:* execute the move/mechanism split per the verdict above ("Constrained selection" pattern + Combobox mechanism). No other entries require splitting.
3. ~~*Concepts role*~~ — removed from scope. Concepts stay in `packages/components/src/stories/concepts/` for now; the extractor's `Concepts`-skip rule stays in place.
