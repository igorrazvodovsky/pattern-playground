...
Lifecycle index for executable specifications. See [README.md](README.md) for
the plan contract and `docs/specs/` for settled specifications.

## Active

- [Agent trace fixtures](active/2026-08-agent-trace-fixtures.md) — outline, needs iteration: extend the shared action log's vocabulary to agent work (intent, search, read, draft, revise as typed actions with provenance), author an in-world fixture trace, rewire the agent reasoning demo to render from it
- [Platform variance: floor-then-variance](active/2026-08-platform-variance.md) — research-gated falsification pilot: an unconditional floor before any variance, functional equivalence as the fence that keeps one language instead of two. The strawman (capability, not screen size) broke — shipping systems replaced device class with *window* size, and WCAG turns out to model input as a floor, not a branch. No schema field until the vocabulary survives the dozen pages that already carry device prose; `editing-in-place`, `selection` and `drag-and-drop` converged on the shape unprompted
...
- [World re-grounding: individuals, values, actions, facts](completed/2026-08-world-regrounding.md) — executed 2026-08-15 to 2026-08-20 across six territories: `shared/world/` is now the authoritative fixture world — 128 bare individuals, 270 hand-authored actions, four declarative rules, and 1282 replay-generated facts — and every entity file under `shared/data/` retired, its shape reproduced exactly as a derived view. `scripts/world/replay.ts` enforces chronology, registration, per-type effect declarations, cardinality, explicit retraction, and that rules fire everywhere they match and nowhere they do not; `verify-views.ts` proved all twelve collections equal to the JSONs they replace, reading each original at the right point in git history. Divergences are named per territory rather than absorbed: end-of-timeline contradictions, references to entries that never existed, and a handoff's misidentification of the assessed e-bike. Concepts deliberately not adopted — the revisit trigger is the world becoming executable, which is also what would converge the two task type families the residue left standing
- [Animation: finish the half-built motion](completed/2026-08-animation-opportunities.md) — executed 2026-08-07: the corpus needed no new motion, only its half-built motion finished — dialog/drawer exits converted from never-rendering keyframes to `allow-discrete` transitions, toast dismissals unified through one exit path with real travel, both collapsibles' panels animating in step with their chevrons (Base UI panel height; `interpolate-size` progressive enhancement for `pp-sections`), three invalid transition declarations repaired, pane spines and overlap shadows fading instead of popping, async pane/demo arrivals bridged with 150ms fades; all surfaces verified in a driven browser, suite green at 260 tests. Phase 3a's `data-motion-ready` gate left unfired pending a feel check
- [Drag and drop: capture as a language entry](completed/2026-07-drag-and-drop.md) — executed 2026-07-25, closed 2026-08-07: the research gate upheld the strawman (one coarse operation-level node), `drag-and-drop.mdx` authored with Scott & Neil variant names, the two-obligation twin (WCAG 2.1.1 + SC 2.5.7), and seven edges, borrowing both view-system demos; six mention sites swept. Phase 4 unfired — a Storybook Utilities entry for the dnd scaffolding triggers on a second consumer; HHN/Buxton `references/` distils uncurated
- [Pane stack: refit to Astro's grain](completed/2026-07-pane-stack-astro-fit.md) — phases 1–3 executed 2026-07-25: demo registry replaced `client:only` islands, static pane partials replaced page scraping, pane 0 rendered static; `reviveAstroScripts` and both DOMParser scraping paths deleted. Phase 4 gate recorded as failed 2026-08-07 — stable Firefox lacks cross-document view transitions, so ClientRouter stays; revisit if Firefox ships them
- [Component testing depth](completed/2026-08-testing-depth.md) — executed 2026-08-07: the story-based Vitest architecture was current but shallow, so the plan added depth rather than architecture. Suite brought green (the contrast floor raised at the colour-system level), every play function given assertions, keyboard and focus sittings for all six components — which found and fixed focus never returning to the trigger, and surfaces leaking between stories — Stylelint tuned to the corpus and joined to the gate (seven dead-CSS bugs it had never been able to report), and a node-environment `unit` project added beside the browser one. `npm run test` passes end to end at 251 tests across both projects. Three deliberate noes, each with a revisit trigger: no CI (local pre-merge habit), no graph-script restructuring, no visual regression
- [React residue onto the platform-first ladder](completed/2026-08-react-to-platform.md) — executed 2026-08-07: hover-card deleted, MermaidDiagram converted and renamed to `pp-diagram` (authored diagrams, as against the chart family's computed ones), PatternGraph swapped for `pp-network-graph` with the site wiring moved into `apps/patterns/src/lib/`; the graph-data mirror under `packages/components/` and its sync check retired with it. Height animation dropped — the CSS premise did not survive contact; sidebar deferred. The React-bound boundary is recorded in the plan
- [Elena migration: the library follows the philosophy](completed/2026-08-elena-migration.md) — executed 2026-08-07: probe landed, rung-2 sweep, rung-3 recompositions (tooltip, timestamp), charts restructured to D3-owns-DOM, Lit retired; conversion recipes recorded in the plan, durable rules promoted to `component-authoring.md` and the component/TS rules; ladder review: `pp-avatar` dissolved to `.avatar` classes, `pp-h` to a `data-heading` mark, remaining candidates open
...
- [Tldraw integration](paused/2025-tldraw-integration.md)
- [Maturity status tags](paused/2026-03-maturity-status-tags.md)
- [Mediation type](paused/2026-03-mediation-type.md)
- [Quality tags from graph](paused/2026-03-quality-tags-from-graph.md)
- [Vertical threads](paused/2026-03-vertical-threads.md)
- [Semantic zoom](paused/2026-semantic-zoom-1.md)

## Superseded

- [Patterns reorganisation](superseded/2025-patterns-reorganization.md) — superseded by Activity Theory reorg
- [Quirky giggling creek](superseded/2026-03-quirky-giggling-creek.md) — superseded by quality-tags-from-graph framing
...
- [Deletion pattern](archive/deletion-pattern.md)
- [Dynamic explanation feature](archive/dynamic-explanation-feature.md)
- [Dynamic explanation status](archive/dynamic-explanation-status.md)
- [Isolated nodes](archive/isolated-nodes.md)
- [pp-sections](archive/pp-sections.md)
- [React modal service enhancement](archive/react-modal-service-enhancement.md)
- [React portals modal implementation](archive/react-portals-modal-implementation.md)
- [Text editor plugin architecture](archive/text-editor-plugin-architecture.md)
- [Universal commenting architecture](archive/universal-commenting-architecture.md)
- [Web component modal enhancement](archive/web-component-modal-enhancement.md)
