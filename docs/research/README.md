...
- A run folder is a durable citation. When a finding gates a decision, the doc recording that decision cites the folder by hand — see the research-gate citations in `docs/language/relationship-vocabulary.md`. Failed runs stay too, so a later run can see what was already tried and set aside.
