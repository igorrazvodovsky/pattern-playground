...
shared/                     Fixture-data workspace (@pattern-plgrnd/shared) — the
├── world/                  fixture world both surfaces render; belongs to neither
└── data/                   language
...
Fixture data both surfaces render, in two layers:

- `world/` — the authoritative substrate: bare individuals, the action log that
  produced them, the relation and action-type registries, declarative rules,
  and a fact set generated by replaying the log. Specified in
  [world-ontology.md](./world-ontology.md).
- `data/` — the derived consumption layer: the entity views assembled from the
  world, finders and bindings over them, and the UI configuration the world
  holds nothing about (status, priority and label display metadata).

Nothing under `apps/` or `packages/` imports `shared/world` directly — the
views are the only surface. New entities and new happenings are added by
authoring actions, never by editing a view or the generated fact set.
