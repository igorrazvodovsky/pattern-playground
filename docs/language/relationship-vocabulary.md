...
The first framing is enforced by a *consumer contract*: no pipeline step — extractor, query function, renderer, or agent tooling — may match, filter, or route on a condition or situation. Situations and hints render as prose for an actor's judgement; once a query filters by one, the hints have become rules. A condition becomes rule-grade when a machine evaluates it, not when it is typed, structured, or stored. Structure is therefore allowed (a situation clause has a shape, a hint has a question and a branch); machine evaluation is not. Where [references/report-pattern-language-formats.md](../../references/report-pattern-language-formats.md) recommends machine-readable, queryable postconditions, this stance governs — see the 2026-07-10 situations changelog entry for the research behind that call.
...
- Authoring test: does B's design take A's result as its material — a structure, a surface, or a newly opened problem — so that removing A from the design leaves B nothing to act on? If the note instead narrates what the actor does next ("what happens after…", "can become part of…"), the claim is runtime-only and does not belong on this type. A note voiced in dataflow or "enables" wording is the same tell one type over — check it against the note-verb advisory.
...
- *Register*: `precedes` speaks in the *designer's* register — the traversal of patterns, where a situation is the history of moves already applied (situation calculus, the pattern-form canon, and Alexander's unfolding all define it this way). The claim is design-time only; the actor's runtime sequence can corroborate it but never ground it — an edge that holds solely as session order fails the authoring test above and comes off the type. One family of coincidences is systematic and stays: in a *protocol family*, where the design object is itself a sequence of moves (the conversation family's encounter), the order the moves are designed in and the order the actor meets them are the same thing, so the edges are genuinely make-time — Alexander's entrance sequence (Entrance Transition → Entrance Room) is ordered by walking order for the same reason. Where the registers diverge — B *replaces* A under a condition rather than acting on A's product ("hub and spoke takes over when items exceed screen capacity") — the plain generative claim is false as stated and true only through the join: *A's resulting context under a condition is B's initiating situation*. Register divergence identifies such an edge as a compressed conditional join, which should be decomposed through the situation constructs (see §Situations) rather than re-typed.
...
- Not `precedes`: the host does not produce a centre the hosted pattern acts on; it merely locates the trigger. Where a note on a `precedes`/`enables` edge reads "container", "canvas", or "where it lives", the edge is usually a hosting claim stored under the wrong type — the note-verb advisory flags these.
...
The vocabulary has entry gates — the SKOS sanity check, the changelog's what-was-considered discipline — but a type also needs a retirement path. The risk being policed is overlap, not number: there is no type-count budget, and a rarely-used type that says something nothing else says (the enacts-shaped bridges) stays however many types there are. A type retires when its distinctions stop being ones authors reach for or readers use.

Standing signals:

- *Per-type counts* in the extractor's summary line, advisory register — a type whose count only ever falls, or whose uses turn out on audit to belong to one other type, is a retirement candidate.
- *`related`'s share of all edges* is the health dial, read as a prompt rather than a failure metric: `related` is the correct home for unclaimed adjacency and stays legitimate at any share; the dial marks where to hunt for conversion opportunities (baseline 25% at 2026-07-10; the extractor prints the current share). The tell is sweep yield, not the share itself — if sweeps into `related` keep converting edges to types, typed claims remain there; if they keep coming back empty, the remaining edges are correctly typed.
...
### 2026-08-27 — Precedes register sweep: make-time only; no use-time edge; the data-view and localization reversals

All 94 `precedes` edges were audited against a single test — does B's design take A's result as its material, so that removing A leaves B nothing to act on? — prompted by a make-time navigation ordering (Alexander's book order) that the raw edge set could not support. 57 passed; 19 held only as the actor's session order; 6 were composition, location, or co-deployment stored under the sequential type; 12 stated a real make-time claim with the arrow reversed. The full per-edge record is in `plans/completed/2026-08-precedes-register-sweep.md`.

Changes: the authoring test above entered the `precedes` definition; the register note no longer offers runtime sequence as the basis of the claim, and names the protocol-family coincidence (the conversation family's encounter edges stay `precedes` because a protocol's design object is itself a sequence). The data-view group reversed — filtering, grouping, and sorting now *follow* data-view, whose framed collection is their material, while searching kept its direction as a resulting clause (a search produces a result set with no view yet in place). The localization group reversed — conversation, generated content, data entry, notification, and help now source `precedes` into localization, consistent with the one edge already voiced carefully (information architecture → localization); localization becomes a late, cross-cutting move. Retypes: embedded intelligence's constituent claims to `enables`, three hosting claims (settings → localization, prompt → suggestion, workspace → activity-log) to `hosts`/`hosted-by`, transparent reasoning ↔ activity log to `complements`, annotation → collaboration to `enables`. Drops: autofill → saving, annotation → conversation, generated-content → suggestion (rerouted through next-best-action, whose initiating situation names the relation the old note could not), searching → navigation-overview's `sets-up` (the fallback stays a bare clause, the pair now `complements`). The one `precedes` cycle (agent → generated-content → suggestion → prompt → agent) was removed by the retypes.

What was considered: a use-time edge type (`then`) for the 19 session-order claims. Rejected for now — the conversation family's edges survive as make-time under the protocol coincidence, and the remainder were too few to earn a type. What was lost: the graph no longer distinguishes session order from design order where both hold, and the encounter's resume loop (disengage → openings) shows only in prose clauses, not as its own relation. If a use-time edge is minted later, the coincident set has to be re-derived — trivial for the conversation family, a handful elsewhere.
...
The absorption entry below rested on a mechanism fact — situations never render on a pattern page, so the prose `## Consequences` section was the clauses' reader-facing render. The backfill then filled 90 pages with resulting clauses while (correctly, under the provisional call) authoring no prose sections, which made the fact absurd in practice: on all but seven pages the "reader-facing render" didn't exist, and 182 of 219 clauses — every one without `sets-up` — reached no reader at all. The fix inverts the component instead of hand-writing ~85 prose sections: `Consequences.astro` renders `situation.resulting` at the page foot before Related patterns, the same never-authored-always-rendered rule the Related list already follows. This keeps one judgement in one authored home with a machine render; the hand-narration alternative was rejected as a second corpus-wide pass whose product drifts (the seven specimens had already drifted to near-verbatim restatement).
...
Three repairs from the precedes sweep's reflection: `enables` softened to "incorporates or builds on" (aligning text with settled practice — Winston's component–integral relation doesn't require necessity); the foundation tiebreaker written down (later grown to four arms); the note-verb advisory added — precision-biased tells for notes whose phrasing claims a different type than the stored one, with deliberate holds kept as standing advisory lines (the advisory line records that the hold is deliberate).
