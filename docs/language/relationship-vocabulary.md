...
*Backfill landed* (2026-07-12). The page-by-page pass sorted 112 ComponentRef mentions across 49 pages. 15 pages now carry a `realised_by` claim (the 3 seeds — form, command-menu, sections — plus 12: tag, item-view, workflow, conversation, notification, bounded-choice, transient-feedback, status-feedback, multilevel-tree, fully-connected, block-based-editor, inline-confirmation), holding 24 ids in all; the remaining 36 ComponentRef pages resolved to citation-only. The keystone was command-menu's own list — 1 of 5 promoted — which set the default: within a `## Related components` list a member is a claim only when it *is this move's mechanism*, and a debatable member stays citation. Three structural disqualifiers did most of the sorting: quality-role pages (agency, formality, malleability, density) are *enacted*, not realised; collection pages (data-visualization, navigation-overview) survey members rather than name a move; a foundation gradient (modality) exhibits its components rather than being realised by them. Two multi-realiser reads go beyond the single-eponymous seeds — notification (toast/callout/dialog, one per `###` type header) and bounded-choice (the control family, one per scale) — both crisp because the page is structured around the set. Calls most open to author override: conversation → messaging (rests on inline-interface's cross-page "the Messaging component is its mechanism" framing over the page's own locative "where conversation happens"), and the drop of Input across data-entry/autocomplete/autofill to citation (a build-on primitive, consistent with the corpus's own gloss). Every id resolved at build.

*Coverage check* — the prose-bounded pass above can only see components a page already `<ComponentRef>`s, so a second pass ran the other direction: every `components-*` docs id against the pattern list. It caught block-based-editor, whose eponymous component realises its move but whose page imports a demo rather than citing the component — invisible to the first pass, added above.

A third direction closed the last gap: the demos (`packages/components/src/demos/`) and the component Storybook docs, which realisation binds neither by prose nor by name. inline-confirmation imports its own demo and never cites a component, and its realiser has an unrelated name — but MorphingControls' doc states outright "the commitment-gathering move built on this mechanism is inline confirmation" with a `<PatternRef>` back-link, so the claim existed component-side and was mirrored to `realised_by` pattern-side. Sweeping every component doc's `<PatternRef>` back-links as a detector then *confirmed* the rest of the pass rather than extending it: only MorphingControls' framing was realisation-grade ("the move built on this mechanism"); every other back-link (combobox→searching, input→data-entry, keyboard-key→keyboard-shortcuts, progress-indicator→wizard, action-bar→its four selection consumers, …) is a component-side "Related patterns" citation, and each agrees with the citation call made pattern-side. The two directions corroborate. (Formalising the reverse back-links as a rendered channel stays out of scope — they served here only as a detector.)

Four components stay orphan (referenced by no page, demo, or doc back-link as a realisation): avatar, list, range, textarea — none maps to an existing pattern's move (range→bounded-choice and textarea→data-entry are the nearest, both left unclaimed: bounded-choice is framed around discrete sets, Range's own doc frames it as a continuum-commit, and Input's whole family reads as a build-on primitive here). Twenty stay cited-only by deliberate judgement (the citation column of the pass).
