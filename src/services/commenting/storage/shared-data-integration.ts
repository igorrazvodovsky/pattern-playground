...
import commentsData from '../../../stories/shared-data/comments.json' with { type: 'json' };

// Interface for the existing comment format from comments.json
interface SharedDataComment {
  id: string;
  threadId: string;
  content: string;
  authorId: string;
...
// Transform existing comment format to universal EntityComment format
function transformSharedDataComment(sharedComment: SharedDataComment): EntityComment {
  return {
    id: sharedComment.id,
    entityType: sharedComment.entityType,
    entityId: sharedComment.entityId,
    content: sharedComment.content, // Keep as string for now
    authorId: sharedComment.authorId,
    timestamp: new Date(sharedComment.timestamp),
    status: sharedComment.status,
    replyTo: sharedComment.replyTo || null,
    metadata: sharedComment.metadata
  };
}

// Load existing comments from shared-data into universal format
export function loadSharedDataComments(): EntityComment[] {
  try {
    return commentsData.map(transformSharedDataComment);
  } catch (error) {
