...
<Meta title="Actions/Application/Cognitive forcing functions/Human goes first" tags={['activity-level:action', 'atomic:pattern', 'role:pattern', 'lifecycle:application', 'mediation:individual']} />
...
- *Relationship to [learnability](../?path=/docs/qualities-learnability--docs).* This is the only forcing function with a genuine skill-building dimension. It connects to [domain learnability](../?path=/docs/qualities-learnability--docs) — the kind that builds competence over time rather than just supporting it temporarily.
