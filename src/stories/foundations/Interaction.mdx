...
Same input, different outcome depending on a state the actor must remember. Design stance following Tesler and Raskin: prefer modeless; when unavoidable, make modes visible, escapable, and hard to forget.

### Types

*Persistent modes* remain active until explicitly dismissed — highest mode-error risk. Edit vs. view, or [focused vs. inline vs. ambient](../?path=/docs/activities-bot--docs) bot interaction.

*Quasimodes* last only while a physical action is maintained (holding a modifier key, long-pressing). The body is the reminder. [Toggle mode in dynamic hyperlinks](../?path=/docs/actions-seeking-dynamic-hyperlinks--docs) is an example. Preferred for temporary mode shifts.

*Transient modes* are system-imposed interruptions blocking the primary workflow (e.g. [dialog](../?path=/docs/actions-application-dialog--docs)).

### Design considerations

- *Visibility*: if discovering the current mode requires performing an action, the mode is invisible.
- *Escapability*: cheap exit always. [Undo](../?path=/docs/operations-undo--docs) mitigates mode errors but doesn't excuse them.
- *Scope*: narrow (single tool) safer than broad (all input).
...
- *[Agency](../?path=/docs/qualities-agency--docs)*: actor-initiated transitions are legible; system-initiated ones risk mode error.

### Modes and intent resolution

The navigation categories above — navigating, browsing, exploring, monitoring — are themselves modes of intent resolution, though transitions are typically fluid rather than discrete state switches. [Collaboration modes](../?path=/docs/foundations-collaboration--docs) and [temporal modes](../?path=/docs/qualities-temporality--docs) follow a similar pattern.
...
- [Wikipedia / Mode](https://en.wikipedia.org/wiki/Mode_(user_interface))