...
<Meta title="Activities/Embedded intelligence" tags={['activity-level:activity', 'atomic:pattern', 'role:pattern', 'mediation:individual']} />
...
- [Adaptability](../?path=/docs/qualities-adaptability--docs) — the system mixes rule-driven and inferred responses, so behaviour shifts with context rather than staying fixed
- [Agency](../?path=/docs/qualities-agency--docs) — the system decides and acts on its own behalf rather than only responding to the actor's explicit moves

### Follow-ups & Complements

- [Command menu](../?path=/docs/actions-seeking-command-menu--docs): A natural home for embedded intelligence, turning a simple command line into an intent-driven interface.
- [Filtering](../?path=/docs/actions-seeking-filtering--docs): A prime example of embedded intelligence, where natural language queries are translated into structured filters.
- [Focus and context](../?path=/docs/actions-evaluation-focus-and-context--docs): Can use embedded intelligence to infer what contextual information is most relevant to a user's current focus.
- [Suggestion](../?path=/docs/actions-application-suggestion--docs): The output of embedded intelligence is often a suggestion that the user can accept, reject, or modify.
