...
If that fails, users can delegate filtering to a [bot](../?path=/docs/patterns-intelligence-bot--docs) via a [task](../?path=/docs/concepts-task--docs). Bot can have access to data beyond the attributes and values visible to the user.
...
- Depends on [command menu](../?path=/docs/patterns-interaction-command-menu--docs) for the local search through attributes and values.
