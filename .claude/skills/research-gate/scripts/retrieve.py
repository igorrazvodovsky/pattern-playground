...
def oa_get(path, params):
    ...
    return json.loads(get(url, retry_wait=5))
...
def cmd_lineage(args):
    ...
    for p in inputs:
        for r in set(p["refs"]):
            ref_owners[r].append(p["title"] or p["given"])
    convergent = sorted(
        ((r, owners) for r, owners in ref_owners.items() if len(owners) >= 2),
    ...
    cite_owners, cite_meta = defaultdict(list), {}
    for p in inputs:
        for c in p["citing"]:
            cid = short_id(c["id"])
            cite_owners[cid].append(p["title"] or p["given"])
            cite_meta[cid] = c
    descendants = [
        {
            "openalex_id": cid,
